...
describe("engine merging logic", () => {
  it.effect(
    "deduplicates backfill rows that were already seen in the live stream",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "publishes all backfill rows when there is no overlap with live",
    () =>
      Effect.gen(function* () {
        const { queue, stream } = yield* makeWebhookQueue<TestRow>({
          capacity: 100,
  ...
  it.effect(
    "publishes nothing from backfill when all rows were already seen live",
    () =>
      Effect.gen(function* () {
        const { queue, stream } = yield* makeWebhookQueue<TestRow>({
          capacity: 100,
