...
No persisted state is `pending`; an incomplete backfill is `backfilling`; completed backfill or changes-only resources are `live`; runtime failures set `error` with `lastError`.

## Wings Contract

Import pure connector manifests from first-party producer packages, for example `@useairfoil/producer-polar/manifest`, through a registry. Use `Manifest.configSchema(manifest)` for server-side validation and `Schema.toStandardSchemaV1(...)` for form resolvers. The schema accepts JSON-shaped config and browser form-shaped values, including stringified numbers/booleans and empty optional fields. Treat `field.required` and `field.default` independently: a default seeds form/runtime defaults, while requiredness controls validation. Store user-facing connector config by manifest field `name`; deploy it by mapping each stored field value to the required `field.env` name and injecting individual environment variables into producer pods. Secret rendering/redaction comes from `field.secret`; existing secret values should not be returned to the browser, and pod injection should use server-side stored secret values, typically through Kubernetes Secrets. Platform-owned runtime config such as Wings host, namespace, table names, and webhook ports remains normal environment config set by infrastructure.
